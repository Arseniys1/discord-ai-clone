  ...
  return (
    <div className="flex-1 flex flex-col bg-[#313338] overflow-hidden">
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
        {messages.map((msg) => (
          <div key={msg.id} className="flex space-x-4 hover:bg-[#2e3035] -mx-4 px-4 py-1 group">
            <img 
              src={msg.isAI ? "https://picsum.photos/id/102/40/40" : "https://picsum.photos/id/64/40/40"} 
              className="w-10 h-10 rounded-full mt-1 shrink-0" 
            />
            <div className="flex-1">
              <div className="flex items-baseline space-x-2">
                <span className={`font-semibold cursor-pointer hover:underline ${msg.isAI ? 'text-blue-400' : 'text-white'}`}>
                  {msg.author}
                  {msg.isAI && <span className="ml-1 px-1 bg-[#5865f2] text-[10px] text-white rounded">BOT</span>}
                </span>
      ...
      <div className="px-4 pb-6">
        <form onSubmit={handleSubmit} className="relative flex items-center bg-[#383a40] rounded-lg px-4 py-2.5">
          <button type="button" className="text-[#b5bac1] hover:text-[#dbdee1] mr-4"><PlusCircle size={24} /></button>
          <input
            ref={inputRef}
            type="text"
